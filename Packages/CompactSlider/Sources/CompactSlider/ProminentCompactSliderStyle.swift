...
public struct ProminentCompactSliderStyle: CompactSliderStyle {
    ...
    public func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(
                // Ice patch: on the macOS 27 SDK, `LinearGradient` satisfies both
                // `View.opacity` and `ShapeStyle.opacity`, so the call below is
                // ambiguous and the package fails to compile. Resolving the gradient
                // through a `Rectangle` fill pins it to the `View` overload. Purely a
                // type-resolution change -- the rendered result is identical.
                // Upstream: https://github.com/buh/CompactSlider/issues/35
                Rectangle().fill(
                    LinearGradient(
