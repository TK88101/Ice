...
/// The children read itself, bounded (hardening plan H2): the live reader asks
/// Accessibility for at most `cap + 1` children through
/// `AXUIElementCopyAttributeValues`, so an oversized snapshot never crosses the
/// process boundary whole. The fetch is a seam; these tests script it.
@Suite("LiveExtrasReader: the bounded children fetch")
struct LiveExtrasReaderChildrenFetchTests {
    private let process = testProcess(pid: 42)
    private var bar: AXUIElement { AXUIElementCreateApplication(getpid()) }

    /// Runs `readChildren` with a scripted fetch, recording what the fetch was
    /// asked for and which children the walk was handed.
    private func read(
        cap: Int = LiveExtrasReader.maxChildren,
        answer: (AXError, CFArray?)
    ) -> (raw: RawRead, fetches: [(index: CFIndex, maxValues: CFIndex)], walked: [(index: Int, element: AXUIElement)]) {
        var fetches = [(index: CFIndex, maxValues: CFIndex)]()
        var walked = [(index: Int, element: AXUIElement)]()
        let raw = LiveExtrasReader.readChildren(
            of: bar,
            process: process,
            extrasElapsed: 0.011,
            timeout: ReadClassifier.defaultTimeout,
            cap: cap,
            interrupt: { false },
            fetch: { _, index, maxValues in
                fetches.append((index, maxValues))
                return answer
            },
            readChild: { index, element in
                walked.append((index, element))
                return (extrasRecord(childIndex: index, identifier: "c\(index)", minX: Double(100 + 30 * index)), false)
            }
        )
        return (raw, fetches, walked)
    }

    @Test("w8: the fetch is asked for children 0 onward, at most cap + 1 of them; cap + 1 back means over the cap")
    func fetchIsBoundedAtCapPlusOne() {
        let elements = (0...LiveExtrasReader.maxChildren).map { _ in bar }
        let (raw, fetches, walked) = read(answer: (.success, elements as CFArray))

        #expect(fetches.count == 1)
        #expect(fetches.first?.index == 0)
        #expect(fetches.first?.maxValues == 65)
        #expect(walked.isEmpty)
        #expect(raw.walkInterrupted)
        #expect(raw.childCount == 65)
        #expect(ReadClassifier.outcome(raw) == .failed(.walkInterrupted))
    }

    @Test("w9: the fetched array reaches the walk untouched -- every element, in order, and its count")
    func fetchedArrayReachesTheWalkUntouched() {
        let elements = [AXUIElementCreateApplication(getpid()), AXUIElementCreateApplication(1), AXUIElementCreateApplication(2)]
        let (raw, _, walked) = read(answer: (.success, elements as CFArray))

        #expect(walked.map(\.index) == [0, 1, 2])
        #expect(zip(walked.map(\.element), elements).allSatisfy { CFEqual($0, $1) })
        #expect(raw.childCount == 3)
        #expect(raw.records.count == 3)
        #expect(raw.walkInterrupted == false)
        #expect(raw.childrenError == "success")
        #expect(raw.extrasElapsed == 0.011)
    }

    @Test("w10: a fetch error maps through AXErrorNames exactly as the single-value read did", arguments: [
        AXError.cannotComplete, .noValue, .attributeUnsupported, .illegalArgument, .failure, .apiDisabled,
    ])
    func fetchErrorMapsAsBefore(error: AXError) {
        let (raw, _, walked) = read(answer: (error, nil))

        #expect(raw.childrenError == AXErrorNames.name(error))
        #expect(raw.extrasError == "success")
        #expect(raw.records.isEmpty)
        #expect(raw.childCount == 0)
        #expect(raw.walkInterrupted == false)
        #expect(raw.childrenElapsed != nil)
        #expect(walked.isEmpty)
    }
}
