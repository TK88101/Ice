...
struct StandInDividerDiscoverer: Discovering {
    ...
    func discover(previous: DiscoveredItemSet?) async -> DiscoveryResult? {
        ...
        return DiscoveryResult(set: stoodIn, duration: result.duration, origin: result.origin, bounds: result.bounds, nextCursor: result.nextCursor, quarantined: result.quarantined, enumeratedPIDs: result.enumeratedPIDs)
    }
}
